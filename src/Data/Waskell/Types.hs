...
newtype Scop a = Scop (a, [NewScope])
...
instance Typeable (Scop Expression) where
  ...
  getType (Scop (airExpr, _)) = getType PairNull
...
checkTypes given@(Scop (e, _)) required = do
  a <- getType given
  b <- getType required
  unless (a == b) $ void expError b a e

subType :: (WaccTypeable a, Referenceable a) => Pos a -> [Type] -> ErrorList Type
subType op ts = do 
  (t, ts') <- subType' (getWType op) ts [] op (getWType op) 1 (getPos op)
  if null ts' then return t else fail "Too many elements when attempting to do a type substitution"
...
subType' (ArrayWaccType (t :-> RetWT) ::> ws) (Pairable (ArrayType t') : ts) tids op opW track pos
  | t == t' = if ws /= RetWT then subType' ws ts tids op opW track pos else return (t, ts)
  | ws /= RetWT = subType' ws ts tids op opW track pos >>= \res -> throwTypeError res pos ("Failed to match array types in argument " ++ show track ++ " of operator " ++ getName op ++ " with type " ++ show opW ++ ". We required [" ++ show t' ++ "] but were given [" ++ show t ++ "]")
  | otherwise = die TypeStage pos ("Type Error: " ++ "Failed to match array types in argument " ++ show track ++ " of operator " ++ getName op ++ " with type " ++ show opW ++ ". We require [" ++ show t' ++ "] but were given [" ++ show t ++ "]") 200

subType' (ArrayWaccType (t :-> RetWT) ::> ws) (t' : ts) _ op opW track pos
  | t == t' = return (t, ts) 
  | otherwise subError ts (Pairable (ArrayType t)) t' op opW track pos
