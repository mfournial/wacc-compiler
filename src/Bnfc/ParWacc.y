...
Function : Type Identifier '(' ListParameter ')' 'is' FunListStatement 'end' { (Function $1 $2 $4 (mkSB $7), getPos $2) }
...
FunListStatement :: { [Statement] } -- ^ non empty list of at least one return statement
...
LimitedStatement : 'skip' { StatSkip }
                 ...
                 | PrintStatement { $1 }
                 | PrintlnStatement { $1 }
                 | 'if' Expression 'then' FunListStatement 'else' LimitedListStatement 'fi' { StatIf $2 (mkSB $4) (mkSB $6) } -- ^ 
                 | 'if' Expression 'then' LimitedListStatement 'else' FunListStatement 'fi' { StatIf $2 (mkSB $4) (mkSB $6) }
                 | 'if' Expression 'then' LimitedListStatement 'else' LimitedListStatement 'fi' { StatIf $2 (mkSB $4) (mkSB $6) }
...
ExpressionBool : ExpressionBool OrT ExpressionBool1 { (BExp $1 (BOr, $2) $3, $2) }
               | ExpressionBool1 { $1 } 
ExpressionBool1 :: { Pos Expression }
ExpressionBool1 : ExpressionBool1 AndT ExpressionBool2 { (BExp $1 (BAnd, $2) $3, $2) }
                | ExpressionBool2 { $1 }
ExpressionBool2 :: { Pos Expression }
ExpressionBool2 : ExpressionBool2 EqT ExpressionBool3 { (BExp $1 (BEqual, $2) $3, $2) }
...
ExpressionTerm : ExpressionTerm MinusToken ExpressionFactor { (BExp $1 (BMinus, $2) $3, $2) }
               | ExpressionTerm PlusToken ExpressionFactor { (BExp $1 (BPlus, $2) $3, $2) }
               | ExpressionFactor { $1 }
ExpressionFactor :: { Pos Expression }
ExpressionFactor : ExpressionFactor TimesT  ExpressionValue { (BExp $1 (BTimes, $2) $3, $2) }
                 | ExpressionFactor DivideT ExpressionValue { (BExp $1 (BDivide, $2) $3, $2) }
                 | ExpressionFactor ModuloT ExpressionValue { (BExp $1 (BModulus, $2) $3, $2) }
                 | ExpressionValue { $1 }
