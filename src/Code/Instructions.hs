...
instance PrintARM Instr where
  ...
  printARM (AND  cond set reg oReg op2)   = 
    if op2 == (ImmOpInt 0)
      then ""
      else "\t\t" ++ "AND" ++ printARM(cond) ++ printARM (set) ++ " " ++ printARM (reg) ++ ", " ++ printARM(oReg) ++ ", " ++ printARM(op2)
